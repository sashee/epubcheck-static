...
const {major, minor, patch, libVersion} = packageJson.version.match(/^(?<major>[^.]+)\.(?<minor>[^.]+)\.(?<patch>[^.]+)-(?<libVersion>.*)$/).groups;

const newVersion = await (async () => {
	console.log("Current libVersion version: " + libVersion);

	const releasesUrl = "https://api.github.com/repos/w3c/epubcheck/releases";
	...
	const currentReleaseCreatedAt = allReleases.find(({tag_name}) => tag_name === libVersion)?.created_at;
	if (!currentReleaseCreatedAt) {
		throw new Error("Could not find the current release for tag: " + libVersion);
	}
...
if (newVersion) {
	const newPackageVersion = newVersion === libVersion ? `${major}.${parseInt(minor) + 1}.${patch}-${newVersion}` : `${parseInt(major) + 1}.0.0-${newVersion}`;
